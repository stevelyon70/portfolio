...
<cfif isDefined("url.reset")>
	...
	<cfquery datasource="#application.dataSource#">
		...
		<cflog application="yes" file="pbt_emailPrefs" type="Information" text="User Reset Defaults #session.auth.userid#" />
	<cfabort>
</cfif>
...
<cfif isDefined("form.emails")>
	...
	<cfquery name="updatebuser" datasource="#application.dataSource#">
		...
		<cflog application="yes" file="pbt_emailPrefs" type="Information" text="User Prefs Update: Email #eml# #userid#" />
</cfif>
...
<cfquery datasource="#application.dataSource#">
	...
	values
	(<cfqueryPARAM value = "#url.userid#" CFSQLType = "CF_SQL_INTEGER">, '#form.emails#','#form.savedSrchEmail#','#form.project_stage#','#form.projecttype#',1,<cfqueryparam value="#Now()#" cfsqltype="cf_sql_timestamp">,'#form.amount#','#form.state#')<!---'#datetimeformat(now(), "yyyy-mm-dd HH:mm:ss")#'--->	
</cfquery>	
</cftransaction>
...
<cfset form.tags = listappend(form.tags,form.structures)>
	<cfloop list="#form.tags#" index="_t">
		<cfquery  datasource="#application.dataSource#">
			...
			values
			(#url.userID#, #_t#, 1, <cfqueryparam value="#Now()#" cfsqltype="cf_sql_timestamp">)
		</cfquery>	
	</cfloop>

</cftransaction>
	<cflog application="yes" file="pbt_emailPrefs" type="Information" text="User Prefs Update: Settings #userid#" />
