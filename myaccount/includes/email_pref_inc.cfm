...
<cfelse>
	...
	<cfif checkprefs.recordcount eq 0>
		<cfquery name="qUserTags" datasource="#application.dataSource#">
			...
			values
			(<cfqueryPARAM value = "#session.auth.userid#" CFSQLType = "CF_SQL_INTEGER">, 1,1,'1,2,3,4,7,8,9',3,1,'#datetimeformat(now(), "yyyy-mm-dd hh:mm:ss")#',1,66,'#session.auth.siteID#')	
		</cfquery>
		...
		<cfquery  datasource="#application.dataSource#">
			...
			values
			(#session.auth.userid#, '10000', 1, '#dateformat(now(), 'yyyy-mm-dd')#','#session.auth.siteID#')
		</cfquery>		
	</cfif>
	<cfquery name="get_user1" datasource="#application.dataSource#">
		...
		where bid_users.userid = #session.auth.userid# 
			...
			and pref.siteid = <cfqueryPARAM value = "#session.auth.siteID#" CFSQLType = "CF_SQL_INTEGER">
	</cfquery>
...
<cfif #session.auth.userid# eq 15210 or #session.auth.userid# eq 14601 or #session.auth.userid# eq 2331 >
	<div class="page-header">				
