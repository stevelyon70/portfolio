...
<cfcomponent output="false">
  ...
  <cffunction name="onRequestStart" output="false" returnType="void">
	...
	<cfparam name="session.auth.access" default="basic" />
   ...
   <cfif isUserLoggedIn()>
	...
	<!---run login check, if not coming from pbt or is not on auth page--->
	  <cfif not StructKeyExists(session,"auth") or (isdefined("session.auth.access") and session.auth.access NEQ "basic") or not isdefined("url.contractor") or not isdefined("url.userID")>
