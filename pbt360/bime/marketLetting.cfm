...
<script src="../vendor/echarts/dist/echarts.min.js"></script>
    ...
    <script type="text/javascript">
		function init_mlIndBidschart() {
			...
			//console.log('init_mlIndBidschart');
			//echart Pie		  
			if ($('#mlIndBids').length ){  
				var theme = {
				  color: [
					  '#83bff6', '#188df0', '#BDC3C7', '#3498DB',
					  '#9B59B6', '#8abb6f', '#759c6a', '#bfd3b7'
				  ]
				};			

			  var vlChart = echarts.init(document.getElementById('mlIndBids'),theme);
			  ...
			  vlChart.setOption({
				...
				$.get('bime/process.cfm?main=mktLet&sub=indBids&bDate=<cfoutput>#bDate#&eDate=#eDate#</cfoutput>').done(function (data) {
					vlChart.hideLoading();
		...
		function init_commBidschart() {
			...
			//console.log('init_commBidschart');
			//echart Pie		  
			if ($('#mlIndBids').length ){  
				var theme = {
				  color: [
					  '#fc8080','#f94040','#9B59B6', '#8abb6f'
				  ]
				};			

			  var stChart = echarts.init(document.getElementById('commBids'),theme);
