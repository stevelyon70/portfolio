...
<cfset request.filterUrl = '/?performance_waste'/>
...
<script src="../vendor/echarts/dist/echarts.min.js"></script>
    ...
    <div class="container">
		<section>
			<header class="pageTitle">Market Performance Awards [Water/Waste Treatment] - <cfoutput>#dateformat(month1, 'mmm yyy')# - #dateformat(month3, 'mmm yyy')#</cfoutput></header>
		</section>
		<section>
    ...
    <script type="text/javascript">
		function init_volumechart() {
			...
			if ($('#award_volume').length ){ 
				...
				$.get('bime/process.cfm?main=mpaw&sub=volumeByMonth&bDate=<cfoutput>#bDate#</cfoutput>').done(function (data) {
						vlChart.hideLoading();
		...
		function init_contractchart() {
			...
			if ($('#award_contract').length ){ 
				...
				// Asynchronous data loading 
				$.get('bime/process.cfm?main=mpaw&sub=valueByMonth&bDate=<cfoutput>#bDate#</cfoutput>').done(function (data) {
					cnChart.hideLoading();
					_data = JSON.parse(data)
					//console.log(_data);
					cnChart.setOption({
		...
		function init_stateschart() {
			...
			if ($('#award_states').length ){ 
				...
				// Asynchronous data loading 
				$.get('bime/process.cfm?main=mpaw&sub=volumeByState&bDate=<cfoutput>#bDate#&eDate=#eDate#</cfoutput>').done(function (data) {
					stChart.hideLoading();
					_data = JSON.parse(data)
					//console.log(_data);
					stChart.setOption({
