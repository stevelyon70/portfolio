...
<cfparam name="url.xrp" default="0"/>
<cfset _xrp = (865*url.xrp) />
<cfset _xrpPro = (365*url.xrp-365*.27)+(500*url.xrp-500*.57)/>
<body>
<div class="ajaxBox">
...
<div class="currentvalue row">
	<div class="col-sm-3">Bitcoin Cash</div>
	<div class="col-sm-3 bch"></div>
	<div class="col-sm-3 bchnow"></div>
	<div class="col-sm-3 bchup"></div>
</div>
<div class="currentvalue row">
...
<div class="currentvalue row">
	<div class="col-sm-3">Ripple</div>	
	<div class="col-sm-3 rpl"><cfoutput>#url.xrp#</cfoutput></div>
	<div class="col-sm-3 rplnow"><cfoutput>#_xrp#</cfoutput></div>
	<div class="col-sm-3 rplup"><cfoutput>#_xrpPro#</cfoutput></div>
</div>
<div class="currentvalue row">
...
function getBtc(){
	//console.log(Date.now);
	 $.get("https://api.coinbase.com/v2/prices/BTC-USD/spot", function(data, status){
		 ...
		 var now$ = data.data.amount*0.1202;
		 now$ = now$.toFixed(2);
		 var up$ = now$ - 275;
		 up$ = up$.toFixed(2);
	...
	function getBch(){
	//console.log(Date.now);
	 $.get("https://api.coinbase.com/v2/prices/BCH-USD/spot", function(data, status){
		 //var now$ = data.data.amount*0.221;
		 var now$ = data.data.amount*0.2214;
		 now$ = now$.toFixed(2);
		 var up$ = now$ - 0;
		 up$ = up$.toFixed(2);
       $('.bch').html(data.data.amount);
       $('.bchnow').html(now$);
       $('.bchup').html(up$);
...
function getLtc(){
	$.get("https://api.coinbase.com/v2/prices/LTC-USD/spot", function(data, status){
		 var now$ = data.data.amount*7.085;
		 now$ = now$.toFixed(2);
		 var up$ = now$ - 1072.74;
		 up$ = up$.toFixed(2);
...
function getTot(){
	tot$ = parseFloat($('.btcup').html()) + parseFloat($('.ltcup').html()) + parseFloat($('.ethup').html()) + parseFloat($('.rplup').html()) + parseFloat($('.bchup').html());
	$('.totalup').html('$'+tot$);
	totern$ = parseFloat($('.btcnow').html()) + parseFloat($('.ltcnow').html()) + parseFloat($('.ethnow').html())+ parseFloat($('.rplnow').html())+ parseFloat($('.bchnow').html());
