				...
				<cfif get_tags.recordcount GT 0>
					<div class="row">	
						<div class="col-sm-12">	
							<hr style="border-top: 1px solid ##8c8b8b;">		 	
							<h5>RELEVANT TAGS</h5> 
							<cfloop list="#valuelist(get_tags.tag)#" index="_tag">
							<cfquery name="qTagID" datasource="#application.datasource#" result="tg">
								select tagID, tag
								from pbt_tags
								where active = 1 and tag = '#trim(_tag)#'
							</cfquery>
								<a href="#request.rootpath#search/?search_history=1&action=sresults&state=66&projecttype=1&project_stage=1&project_stage=2&project_stage=3&project_stage=4&tags=#qTagID.tagID#"><span class="tags pull-left">
									#_tag#</span></a>
							</cfloop>
						</div>
					</div>
