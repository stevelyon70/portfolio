...
--tags
    and b.tagID in (#scopes#,#valuelist(qTagsDefaultList.tagID)#)
	and a.status in (3,5)
<!--- remove dupes--->
	<cfif listlen(dupList)>and a.bidid not in(#dupList#)</cfif>
 order by promas.stage, a.submittaldate
...
--tags
    and ppmc.tagID in (#scopes#,#valuelist(qTagsDefaultList.tagID)#)
	and a.status in (3,5)
<!--- remove dupes--->
	<cfif listlen(dupList)>and a.bidid not in(#dupList#)</cfif>
	and promas.tags is not null
