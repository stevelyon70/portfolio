...
where pbt_project_master_gateway.stateid in(#states#) 
	and pbt_project_master_gateway.bidid in (select bidid from pbt_project_clipboard where userID = <cfqueryPARAM value = "#session.auth.userid#" CFSQLType = "CF_SQL_INTEGER">) 
union 
...
where a.stateid in (#states#) 
	and a.bidid in (select bidid from pbt_project_clipboard where userID = <cfqueryPARAM value = "#session.auth.userid#" CFSQLType = "CF_SQL_INTEGER">) 
