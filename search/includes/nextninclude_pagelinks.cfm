...
<cfif isdefined("col")><cfset tempHRef = tempHRef & "&col=bidall&col=btnew&col=btawards&col=indbids"></cfif>
...
<cfif TotalRows GT RowsPerPage>
	Page&nbsp;
	<select name="selPaging" id="selPaging" class="selPaging">
	...
	<cfloop from="1" to="#TotalRows#" Step="#RowsPerPage#" index="PageRow" >
		...
		<cfset IsCurrentPage = (PageRow GTE URL.StartRow) AND (PageRow LT EndRow)>
		<!---if this "page" is current page, show without link--->
		<cfif IsCurrentPage>
		  <!---<B>#ThisPage#</b>--->
		  <!---Otherwise, show link so user can go to page--->
		  <option value="" disabled selected>#ThisPage# : #PageRow#-#(pagerow+rowsperpage-1)#</option>
		<cfelse>
			<cfset pageendrow = pagerow + rowsperpage>
			<!---<a href="#cgi.script_name#?StartRow=#PageRow#&endrow=#pageendrow#&rowsperpage=#rowsperpage#&#tempHRef#">#ThisPage#</a>	--->
			<option value="#cgi.script_name#?StartRow=#PageRow#&endrow=#pageendrow#&rowsperpage=#rowsperpage##tempHRef#">#ThisPage# : #PageRow#-#pageendrow-1#</option>
		</cfif>	
	...
	<!---cfif thispage gt 5>
		<cfset endrow = endrow + rowsperpage>
		<cfset prevrowstart = startrow - rowsperpage>
		<cfset prevrowend = endrow - rowsperpage>
		<cfif startRowNext GT 40>
		<cfif isdefined("sort")>
			<a href="?startrow=#prevRowstart#&endrow=#prevrowend#&sort=#sort#&userid=#userid#&desc=#desc#&tempHRef=#tempHRef#">
		<cfelse>
			<a href="?startrow=#prevRowstart#&endrow=#prevrowend#&userid=#userid#&tempHRef=#tempHRef#">
		</cfif>
			<img src="//app.paintbidtracker.com/projectsearch/images/goprev.gif" alt="back #rowsperpage# records" border="0">
		</a>
		</cfif>
		<cfif isdefined("sort")>
			<a href="?startrow=#StartRowNext#&endrow=#endrow#&sort=#sort#&userid=#userid#&desc=#desc#&tempHRef=#tempHRef#">
		<cfelse>
			<a href="?startrow=#StartRowNext#&endrow=#endrow#&rowsperpage=#rowsperpage#&userid=#userid#&tempHRef=#tempHRef#">
		</cfif>
			<img src="//app.paintbidtracker.com/projectsearch/images/gonext.gif"  alt="next #rowsperpage# records" border="0">
		</a>
	</cfif--->
</cfif>
</cfoutput>
<script>
	$('.selPaging').on('change', function(){
		location = this.value;
	})
