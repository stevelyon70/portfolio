				...
				<tr class="grid projectBid main-row">
				  ...
				  <td class="grid col-xs-3">
					...
					<!---<div> --class="hidden-xs hidden-sm"--
					 ...
					 <cfif url.newSolr eq 1 or url.newSolr eq 3>
						 <cfset hasPDf = application.searchObj.doesPdfExist(bidid) />
					 </cfif>
					 BidID - <span id="bid">#bidid#</span> &nbsp; <cfif hasPDf EQ "YES"><i class="fa fa-file-pdf-o dv" data-bidid="#bidid#" aria-hidden="true"></i></cfif><br>
...
<!---Doc Preview Modal --->
<div class="modal fade" tabindex="-1" role="dialog" id="docview">
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
				<h5 class="modal-title dv-title" id="detailLabel"></h5>
			</div>							  
			<div class="modal-body dv-body"></div>
			<div class="modal-footer dv-footer"></div>
		</div>
	</div>
</div>	
...
$(function () {	
	...
	$('#results_table').on( 'draw.dt', function () {
		...
		// Doc Preview hover
		$(".dv").hover(function() {
			$(this).css('cursor','pointer').attr('title', 'This icon indicates PDF documents are present');
		});
		...
		$(".dv").click(function(){
		  ...
		  var full = "<a href='../leads/?bidid=" + bidID + "&keywords=<cfoutput>#url.qt#</cfoutput>'>View full project details</a>"
		  $(".dv-body").html("Content loading please wait...  <img src='../assets/images/spinner.svg'>");
	  	  $(".dv-title").html("DOCUMENT PREVIEW<br>BidID: " + bidID);

	          $(".dv-footer").html(full);		
	          $("#docview").modal("show");
	          $(".dv-body").load('../leads/includes/docview.cfm?bidid=' + bidID + "&kw=<cfoutput>#encodeForURL(url.qt)#</cfoutput>");
		});		
	...
	$(".dv").click(function(){
	  ...
	  $(".dv-body").load('../leads/includes/docview_SOLR.cfm?bidid=' + bidID + "&kw=<cfoutput>#encodeForURL(url.qt)#</cfoutput>");
		<cfelse>
	  $(".dv-body").load('../leads/includes/docview.cfm?bidid=' + bidID + "&kw=<cfoutput>#encodeForURL(url.qt)#</cfoutput>");
		</cfif>
	});	
