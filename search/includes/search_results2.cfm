...
<cfquery name="checkuserpackage" datasource="#application.datasource#">
	...
	where bid_users.userid =  <cfqueryPARAM value = "#session.auth.userid#" CFSQLType = "CF_SQL_INTEGER"> and bid_subscription_log.packageid in (1,2,3,4,5,6,7,8,12)  and bid_subscription_log.effective_date <= #date# and bid_subscription_log.expiration_date >= #date# and bid_subscription_log.active = 1
</cfquery> 
<!---zipcode filter--->
...
<cfinclude template="sort_params.cfm">
 <cfset total_results.total_returned = 5 />
<cfquery name="search_results" datasource="#application.datasource#" result="r1">
	select *
			...
			FROM    (
			SELECT distinct top 5 g.PhoneNumber,a.bidID,a.projectname,a.owner,a.ownerID,a.tags,a.submittaldate,a.city,
