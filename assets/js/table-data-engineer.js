var TableData = function () {
 ...
 var runAgencyTable = function(){
	...
	var runContractorTable = function(){
     var oTable = $('#contractorT').dataTable({
			"aoColumnDefs" : [{
				"aTargets" : [0]
			}],
			"oLanguage" : {
				"sLengthMenu" : "Show _MENU_ Rows",
				"sSearch" : "",
				"sEmptyTable": "No companies were found.",
				"oPaginate" : {
					"sPrevious" : "",
					"sNext" : ""
				}
		...
		$('#contractorT_wrapper .dataTables_filter input').addClass("form-control input-sm").attr("placeholder", "Search");
		// modify table search input
		$('#contractorT_wrapper .dataTables_length select').addClass("m-wrap small");
		// modify table per page dropdown
		$('#contractorT_wrapper .dataTables_length select').select2();
		// initialzie select2 dropdown
		$('#contractorT_column_toggler input[type="checkbox"]').change(function() {
			/* Get the DataTables object again - this is not a recreation, just a get of the object */
			var iCol = parseInt($(this).attr("data-column"));
			var bVis = oTable.fnSettings().aoColumns[iCol].bVisible;
			oTable.fnSetColumnVis(iCol, ( bVis ? false : true));
	...
	//Agency serverside processing main search page
		var runAgencySearchTable = function(){
     		var oTable = $('#search_AgencyresultsTable').dataTable({	
		        ...
		        "processing": true,
		        "serverSide": true,
		        "sAjaxSource": "../cfc/engineer_profile.cfc?method=pull_engineer_results",
				...
				"fnRowCallback": function( nRow, aaData, iDisplayIndex ) {  
							...
							$('td:eq(0)', nRow).html('<a href="?engineer&engineerID='+aaData[1]+'">'+aaData[2]+'</a>');  return nRow;
							},
				 ...
				 "fnServerData":function (sSource, aoData, fnCallback) {
		                            //aoData.push({"name":"year_field", "value":$("[name='year_field']:checked").val() });
									 ...
									 aoData.push({"name":"specification_field", "value":$("[name='specification_field']").val() });
