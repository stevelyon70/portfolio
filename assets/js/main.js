...
var Main = function() {
	...
	var runNavState = function(){
		
		try{
			var navIs = localStorage.getItem('navState', 1);
		}catch(r){			
	...
	//Set of functions for Filter Selector
	var runFilterSelector = function() {
		$('.filter-toggle').on('click', function() {
			if($(this).hasClass('open')) {
				$(this).removeClass('open').addClass('close');
				$('#filter_selector_container').hide();
			} else {
				$(this).removeClass('close').addClass('open');
				$('#filter_selector_container').show();
			}
		});
		setColorScheme();
		setLayoutStyle();
