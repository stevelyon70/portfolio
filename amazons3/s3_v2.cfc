<cfcomponent name="s3_v2" displayname="Amazon S3 REST Wrapper v1.8">
...
--->
	...
	<cffunction name="init" access="public" returnType="s3_v2" output="false"
	...
	<cffunction name="getObject" access="public" output="false" returntype="string" 
		...
		<!--- Create a proper signature --->
		<cfset var signature = createSignature(cs)>
		<!---old link to amazon -s3.amazonaws.com/#arguments.bucketName#--->
		<!--- Create the timed link for the image --->
		<cfset timedAmazonLink = "http://docs.paintbidtracker.com/#arguments.bucketName#/#arguments.fileKey#?AWSAccessKeyId=#URLEncodedFormat(variables.accessKeyId)#&Expires=#epochTime#&Signature=#URLEncodedFormat(signature)#">

		<cfreturn timedAmazonLink>
	</cffunction>
	
	<cffunction name="getObject2" access="public" output="false" returntype="string" 
				description="Returns a link to an object.">
		<cfargument name="bucketName" type="string" required="yes">
		<cfargument name="fileKey" type="string" required="yes">
		<cfargument name="minutesValid" type="string" required="false" default="60">
		...
		<!---old link to amazon -s3.amazonaws.com/#arguments.bucketName#--->
		<!--- Create the timed link for the image --->
		<cfset timedAmazonLink = "http://document.paintbidtracker.com/#arguments.fileKey#?AWSAccessKeyId=#URLEncodedFormat(variables.accessKeyId)#&Expires=#epochTime#&Signature=#URLEncodedFormat(signature)#">
